...
class Move implements MoveInterface
{
    ...
    public function to(string $to)
    {
        ...
        } else if (!$fromFigure->canMoveTo($toX, $toY, false)) {
        ...
        if (
            $fromFigure instanceof King
            && $fromFigure->isCheckOnPosition($toX, $toY))
        {
            throw new MoveException("You cannot attack this figure by king");
        }

        if ($this->player->getKing()->isCheck()
            && !$fromFigure instanceof King) {
            throw new MoveException("You cannot move figure because of check");
        }
        ...
        if ($this->player->checkForWin()) {
            throw new PromotionException("{$this->player->id()} won game");
        }
    ...
    protected function handleCastling(FigureInterface $figure, string $x, int $y)
    {
        ...
        if (!$castlingFigure->canMoveTo($castlingX, $castlingY, false)) {
